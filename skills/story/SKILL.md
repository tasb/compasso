...
Build one story end to end. Scripts are in `${CLAUDE_PLUGIN_ROOT}/bin`, roles in `${CLAUDE_PLUGIN_ROOT}/roles`. `RUN` is `.compasso/runs/<iid>` in the repo. Every role runs as a subagent that reads its role file, with the model set for that role and this harness in `.compasso/project.yaml`. Pass paths, not recaps.
...
7. **Review loop, at most 3 rounds.**
   ...
   - Otherwise dispatch the **builder** with the open blocker and major findings and every open security finding, then repeat step 5's checks, and start the next round on the new diff: the reviewer re-reads only the changed hunks; security re-checks each of its findings and alone may set `verified_by: security`.
...
8. **Ship.** Dispatch the **shipper** with `RUN/story.json` and `RUN/findings.json`: it commits, writes `RUN/changes.md`, and files each open minor finding as a follow-up under the story's feature (`gitlab.sh followup`). Push the branch.
