...
Harden one delivered feature, or every feature of an epic. It runs only when a person asks and never slows delivery. Scripts are in `${CLAUDE_PLUGIN_ROOT}/bin`, roles in `${CLAUDE_PLUGIN_ROOT}/roles`; each role runs as a subagent with the model set for it in `.compasso/project.yaml`. `RUN` is `.compasso/runs/harden-<iid>`; every check writes its result to `RUN/results/<check>.json` in the common format that `bin/harden-report.sh` describes.
...
4. **Property-based tests.** On a branch `harden/<feature>-properties`, dispatch the **tester** to write property-based tests from the acceptance, for the code in `RUN/scope.patch`. Run them. Rules that hold stay as tests and ship like a story (review and mandatory security review, shipper, merge request "Add property-based tests for #<feature>"). Each rule that breaks is a bug (step 8). Result: counts "rules", "hold", "broken"; one gap per broken rule.
